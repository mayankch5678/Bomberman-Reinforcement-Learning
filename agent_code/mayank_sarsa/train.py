...
SARSA on top of a LINEAR function approximator. Identical to mayank_agent in
features, rewards and hyperparameters; the ONLY difference is the update rule:

    Q(s, a) = w_a . phi(s)

    Q-learning (mayank_agent):  td = r + gamma * max_a' Q(s', a')  -  Q(s, a)
    SARSA      (here):          td = r + gamma *        Q(s', a')  -  Q(s, a)

where a' is the action the policy ACTUALLY took in s' -- exploration included.
That makes it on-policy: the value it learns is the value of the epsilon-greedy
policy being followed, not of the greedy policy it would like to follow.

Getting a' out of the framework
-------------------------------
game_events_occurred(s, a, s') never sees a'. Two ways round it:

  (a) cache the next action inside callbacks.act(), or
  (b) defer each update by one step.

This uses (b). The framework calls game_events_occurred once per step with
(s_k, a_k, s_k+1), so the NEXT call arrives as (s_k+1, a_k+1, s_k+2) -- its
`self_action` argument IS the a' the previous transition was waiting for, by
construction. Nothing needs caching and no extra state crosses module
boundaries. Option (a) was rejected because act() also runs outside training
(and on every greedy evaluation), so making it mutate learning state would put
training side effects on the inference path.
"""
...
def setup_training(self):
    ...
    # (s, a, r, s') held back until the action taken in s' is known.
    self.pending = None
...
def game_events_occurred(self, old_game_state, self_action, new_game_state, events):
    ...
    # `self_action` was taken in `old_game_state`, which is exactly the state
    # the pending transition ended in -- so it is that transition's a'.
    flush_pending(self, next_action=self_action)
    self.pending = (old_game_state, self_action, reward, new_game_state)
...
def end_of_round(self, last_game_state, last_action, events):
    ...
    # Two shapes of ending, and they need different treatment:
    #
    #  - The agent DIED this step. game_events_occurred is skipped for dead
    #    agents, so the pending transition is the one leading INTO
    #    last_game_state, and last_action really was taken there: a genuine
    #    SARSA bootstrap, and the only place that final action is ever learned
    #    from as an a'.
    #  - The round simply ran out. game_events_occurred already fired for
    #    (last_game_state, last_action), so that pair IS the pending
    #    transition. Nothing follows it, so it terminates without bootstrap.
    #
    # Both states belong to the same round, so the step counter separates them.
    if self.pending is not None and self.pending[0]['step'] < last_game_state['step']:
        flush_pending(self, next_action=last_action)
    else:
        flush_pending(self, next_action=None)

    # The terminal transition itself: no future value to bootstrap from.
    update_weights(self, last_game_state, last_action, reward, None, None)
...
def flush_pending(self, next_action):
    """
    Apply the deferred update now that the following action is known.

    `next_action` None means nothing follows the pending transition, so it is
    treated as terminal.
    """
    if self.pending is None:
        return
    old_state, action, reward, new_state = self.pending
    self.pending = None
    update_weights(self, old_state, action, reward, new_state, next_action)


def update_weights(self, old_state, action, reward, new_state, next_action):
    if action is None:
        return
    ...
    if new_state is None or next_action is None:
        q_target = reward
    else:
        # SARSA: bootstrap on the action actually taken, NOT max over actions.
        phi_next = state_to_features(new_state)
        a_next = ACTIONS.index(next_action)
        q_target = reward + GAMMA * (self.weights[a_next] @ phi_next)
